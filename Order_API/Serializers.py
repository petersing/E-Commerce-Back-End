...
class Make_Order_Serializers(serializers.Serializer):
    ...
    DeliveryList = ("Free Shipping", "Free Shipping", "Speed Shipping")
    ...
    def Checking_And_Update_SubItem(self, Data, Author, validated_data):
        ...
        ProductList = Product.objects.filter(id__in = [x['id'] for x in sorted_item]).prefetch_related("product_image_set", 'product_description_images_set').order_by('id').all()
